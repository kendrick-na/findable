...
function toEngineResponses(stored: StoredEngineResponse[]): EngineResponse[] {
  return stored.map((r) => ({
    ...
    // 분모를 복원에서 빠뜨리면 재집계 때 상대위치가 사라져 점수가 조용히 내려간다.
...
async function persistBriefingTracking(
  ...
  const usable = Boolean(
    briefing && !(briefing.isStub || briefing.errorMessage)
  );

  if (dualWriteEnabled && organizationId && brandId && usable) {
...
export async function runBriefingForAuditJob(
  ...
  try {
    ...
    // ⚠️ 저장된 응답엔 citedSources가 없어(toEngineResponses가 []로 복원) 재계산하면
    // topCitedDomains가 빈 배열로 유실된다 → 출처 패널 소실 + 노출품질 축 0점
    // (2026-07-30 결함감사 §19 실측). 출처 집계는 빠른 모드 원본을 보존한다.
    const metrics = {
      ...aggregateAudit(toEngineResponses(mergedStored)),
      topCitedDomains: latest.metrics.topCitedDomains ?? [],
    };
