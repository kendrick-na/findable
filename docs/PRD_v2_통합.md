...
**왜 이 팀인가 (founder-problem fit)** `[확인사실]`
- 대표는 **비개발자이나 AI로 다수 서비스를 직접 기획·배포**해온 운영자(1인 + 4 AI 에이전트). "1인이 앱7·패키지20을 배포"가 실행 속도의 증거.
- **마케팅 현장 경험** → "AI가 내 브랜드를 뭐라 말하는지 모른다"는 페인을 **고객으로서 먼저 겪음**. 문제의 실재를 안에서 검증한 팀.
