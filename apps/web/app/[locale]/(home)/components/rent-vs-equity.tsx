...
export const RentVsEquity = ({ locale = "ko" }: Props) => {
  ...
  const lead = isKo
    ...
    : "Search ads and SEO exist only while you keep paying. Stop, and your visibility disappears. That's rent. GEO is different. Once you make AI cite you accurately and often, that visibility carries into the next answer and the next model. Not a burn rate, but equity that accrues to your brand.";
  ...
  // ⚠️ N-44 문구 정정: 목적지가 `/sign-up` 이다(무료 진단 폼 아님). 👤 결정 A.
  const cta = isKo ? "무료로 시작하기" : "Start free";
    ...
    <section className="bg-[var(--findable-canvas)] px-8 pt-8 pb-14 md:pt-10 md:pb-16">
      <div className="mx-auto max-w-[1200px]">
        <div className="mb-10 max-w-[760px]">
          <h2
            className="text-[var(--findable-ink)]"
            style={{
              fontFamily: isKo
        ...
        {/* 대조 2열 */}
        <div className="findable-glass-field grid gap-6 md:grid-cols-2">
          {/* 임대 (좌) — 소멸/무채색 톤 */}
          <div className="findable-glass rounded-xl p-7">
            <div
            ...
            <Reveal className="mt-5 space-y-3" stagger={60}>
              {rows.map((r) => (
                <div
          ...
          {/* 적립 (우) — 축적/프라이머리 톤 */}
          <div className="findable-glass findable-glass-primary relative overflow-hidden rounded-xl p-7 before:pointer-events-none before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-gradient-to-r before:from-transparent before:via-[var(--findable-primary)]/40 before:to-transparent">
            <div
